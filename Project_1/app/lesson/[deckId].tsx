        ...
        try {
            ...
            if (sessionData && (sessionData.newWords?.length > 0 || sessionData.reviewWords?.length > 0)) {
                router.push({
                    ...
                    params: {
                        words: JSON.stringify(sessionData),
                        deckName: deckDetails?.name || 'Lesson',
                        deckId: deckId
                    },
                });
            } else {
                alert(sessionData.message || "Chúc mừng! Bạn đã học hết từ trong chủ đề này.");
            }
        } catch (error) {
            setIsLoading(false);
            console.error("Lỗi khi bắt đầu bài học:", error);
            alert("Không thể bắt đầu bài học. Vui lòng thử lại.");
        }
    };
                ...
                <View style={styles.wordGridContainer}>
                    {deckDetails.words.map((item) => (
                        <TouchableOpacity
                            ...
                            onPress={() => router.push(`/word/${item._id}`)}
                        >
